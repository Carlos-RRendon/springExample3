package com.FirstServer.App.springairline.rest;
...
import com.FirstServer.App.springairline.model.Flight;
import com.FirstServer.App.springairline.model.Passenger;
...
public class FlightsRestService {
    ...
    @Operation(summary = "Info",description = "Flight info", tags = {"FlightRestService"})
    @GetMapping("/info/{id}")
    public @ResponseBody String getfligthInfo(
        ...
        logger.debug("Rest sercive: /info" + id);
        String info;

        try {
            info = aircraftFleet.getFleet().get(id).getFlightInfo();
        }catch (Exception e){
    ...
    public @ResponseBody String postPassenger(
        ...
        try {
            for (Entry<Long,Flight> entry : aircraftFleet.getFleet().entrySet()){
                ...
                if (passenger != null){
                    return entry.getValue().getFlightInfo() + "\n" + passenger.passengerInfo();
                }
            }
        }
        catch (Exception e){
    ...
    @Operation(summary = "Delete flight", description = "Delete an existent flight", tags = {"FlightsRestService"})
